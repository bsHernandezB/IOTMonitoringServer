...
def analyze_data():
    ...
    data = Data.objects.filter(
        base_time__gte=datetime.now() - timedelta(hours=1)).exclude(measurement_id=3)
    ...
    # Nuevo evento luminico
    data = Data.objects.filter(
        base_time__gte=datetime.now() - timedelta(hours=2),
        measurement_id=3)
    aggregationl = data.annotate(check_value=Avg('avg_value')) \
        .select_related('station', 'measurement') \
        .select_related('station__user', 'station__location') \
        .select_related('station__location__city', 'station__location__state',
                        'station__location__country') \
    ...
    for item in aggregationl:
        ...
        if alert:
            if item["check_value"] > max_value:
                message = "ALERT {} {} {}".format(variable, max_value, "amaneció")
                topic = '{}/{}/{}/{}/in'.format(country, state, city, user)
                print(datetime.now(), "Sending alert to {} {}".format(topic, variable))
                client.publish(topic, message)
                alerts += 1
            ...
            if item["check_value"] < min_value:
                message = "ALERT {} {} {}".format(variable, min_value, "anocheció")
                topic = '{}/{}/{}/{}/in'.format(country, state, city, user)
                print(datetime.now(), "Sending alert to {} {}".format(topic, variable))
                client.publish(topic, message)
                alerts += 1
